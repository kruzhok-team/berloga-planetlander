...
float __attribute__((export_name("_GetTime"))) GetTime() { return t; }

int __attribute__((export_name("_GetMapsOffset"))) GetMapsOffset() {
  return (int)maps;
}

void __attribute__((export_name("_SetKeys"))) SetKeys(int u, int d, int r,
                                                      int l) {
  keys.u = u;
  keys.d = d;
  keys.r = r;
...
void __attribute__((export_name("_Reset"))) Reset(int _level, int _bgcolor1,
                                                  int _bgcolor2,
                                                  int _density2color, int x,
                                                  int y) {
...
float __attribute__((export_name("_GetFuel"))) GetFuel() { return s.fuel; }
...
void Thrust(int x, int y, float sx, float sy) {
  ...
  ShipAddForce((keys.r - keys.l) * 1.f, (keys.d - keys.u) * 1.f);
  ...
  for (int j = y - 5; j <= y + 5; j++)
    for (int i = x - 5; i <= x + 5; i++) {
      if (!IsInside(j, i))
        continue;
      float r2 =
          ((float)i - sx) * ((float)i - sx) + ((float)j - sy) * ((float)j - sy);
      fx += vx[j][i];
      fy += vy[j][i];
 ...
 * Calculate timestep
 */
void __attribute__((export_name("_Step"))) Step(double _realtime,
                                                int simulateShip) {
  if (realtime <= 0) {
    realtime = _realtime;
  } else {
