...
class Game {
  constructor(level, onLose, onWin) {
    ...
    this.texture = Array.from({length: 17 * 32}, () => Math.random() * 0.1 + 0.9);
  }
  ...
  InitGraphics(N, M) {
    ...
    this.graphics.fluidctx = this.graphics.fluidcanvas.getContext("2d", {alpha: true});
    ...
    this.graphics.overlayctx = this.graphics.overlaycanvas.getContext("2d", {alpha: true, willReadFrequently: true});
    ...
    this.graphics.boundaryctx = this.graphics.boundarycanvas.getContext("2d", {alpha: true, willReadFrequently: true});
    ...
    this.graphics.collisionctx = this.graphics.collisioncanvas.getContext("2d", {alpha: true, willReadFrequently: true});
    ...
    this.graphics.shipctx = this.graphics.shipcanvas.getContext("2d", {alpha: true});
  ...
  CollisionDetection() {
    ...
    c.drawImage(this.graphics.shipcanvas, this.gameWasm._ShipGetX() * 4. - 18, this.gameWasm._ShipGetY() * 4. - 20);

    let newcollisioncounts = this.CountCollisionPixels(c);

    if (Math.abs(newcollisioncounts.boundary - this.collisionCounts.boundary) > 10) {
        audio.ThrustOff();
        this.gameWasm._Destroyed();
        return;
    }

    if (Math.abs(newcollisioncounts.landingpad - this.collisionCounts.landingpad) > 10) {
        audio.ThrustOff();
  ...
  Loop(onLose) {
    ...
    if (this.gameWasm._IsExploded() && this.gamePhase === GAME_PHASES.PLAY) {
        ...
        if (this.ships > 0) { // Проверяем, что у нас есть корабли
  ...
  Resize() {
    ...
    if (scale.x > scale.y) {
      this.graphics.fluidcanvas.setAttribute('style', 'width: auto; height: ' + (window.innerHeight) + 'px;');
      this.graphics.overlaycanvas.setAttribute('style', 'width: auto; height: ' + (window.innerHeight) + 'px;');
      this.graphics.svg.setAttribute('style', 'width: auto; height: ' + (window.innerHeight) + 'px;');
    } else {
      this.graphics.fluidcanvas.setAttribute('style', 'width: ' + (window.innerWidth) + 'px; height: auto;');
      this.graphics.overlaycanvas.setAttribute('style', 'width: ' + (window.innerWidth) + 'px; height: auto;');
      this.graphics.svg.setAttribute('style', 'width: ' + (window.innerWidth) + 'px; height: auto;');
    }
  }
  ...
  async Main(onLose) {
    ...
    try {
        ...
        this.color = new Uint8Array(importOb.env.memory.buffer, this.gameWasm._GetColorOffset());
        this.boundary = new Uint32Array(importOb.env.memory.buffer, this.gameWasm._GetBoundaryOffset());
        this.maps = new Uint8Array(importOb.env.memory.buffer, this.gameWasm._GetMapsOffset());
  ...
  GetDensityMap(x, y, offset) {
    ...
    for (let jj = j - 3; jj <= j + 3; jj++)
        for (let ii = i - 3; ii <= i + 3; ii++) {
            ...
            if (jj >= 17) continue;
            if (ii >= 32) continue;
            if ((this.maps[(ii >> 3) + jj * 4 + offset] >> 7 - (ii & 7) & 1) === 0) continue;
  ...
  DrawLevel(c, level) {
    ...
    // lastlevel = level;
    c.clearRect(0, 0, this.graphics.boundarycanvas.width, this.graphics.boundarycanvas.height);
    switch (level) {
        case -1:
            c.font = "100px Arial";
        ...
        case 0:
          ...
          this.score = 0;
            ...
            this.FillCenterText(c, "Ваша задача как почтальона — доставлять почту в отдаленные колонии.", 420);
            this.FillCenterText(c, "Используйте клавиши курсора или WASD, чтобы управлять своим почтовым посадочным модулем.", 450);
        ...
        case 7:
          this.DrawMap(c, 6, {r: 0xF4, g: 0x8D, b: 0x4E});
            ...
            for (let i = 0; i < 50; i++) {
                ...
                if ((x < 300) || (x > 800))
                  this.DrawBuilding(c, x, 500 - h, 35, h);
            }
        ...
        case 9:
          this.gameWasm._Reset(
            ...
            c.font = "50px Arial";
            this.FillCenterText(c, "" + Math.floor(ships * 500 + score) + " Баллов", 300)
            break
    }
  }
...
const GameScreen = ({levelNumber, onLose, onWin}) => {
  useEffect(() => {
    ...
    let game = new Game(levelNumber, onLose, onWin);
